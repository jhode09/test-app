(function($) {
    ...
    $.fn.createTablePagination = function(options) {
        ...
        if ($('.pagination-container-' + $tableId).length > 0) {
            $('#pagination-' + $tableId).remove();

            var $paginationParentContainer = $('.pagination-container-' + $tableId).parent();

            if ($paginationParentContainer.children().length > $('.pagination-container-' + $tableId).index() + 1) {
                $paginationParentContainer.children().eq($('.pagination-container-' + $tableId).index() + 1).before($table.clone());
            } else {
                $paginationParentContainer.append($table.clone());
            }
            $('.pagination-container-' + $tableId).remove();
            // $table = $(tableId);
        ...
        if ($jumpPage == true) {
            ...
            $alignPagination = 'justify-content:right';
        }
        ...
        style.setAttribute('id', 'pagination-' + $tableId);
    ...
    `;
        ...
        if ($parentContainer.children().length > $table.index() + 1) {
            $parentContainer.children().eq($tableIndex + 1).before('<div class="pagination-container-' + $tableId + '"></div>');

        } else {
            $parentContainer.append('<div class="pagination-container-' + $tableId + '"></div>');
        }

        $table.detach().appendTo('.pagination-container-' + $tableId);
        ...
        $table.find('tr').not(':first').each(function() {
            if ($(this).index() >= ($pageDisplay[0].rowStart - 1)&& $(this).index() <= ($pageDisplay[0].rowEnd-1)) {
                $(this).show();
            } else {
                $(this).hide();
            }
        ...
        function displayPage(thisButton, type) {
            ...
            $table.find('tr').not(':first').each(function() {
                if ($(this).index() >= ($pageDisplay[$currentActivePage - 1].rowStart-1) && $(this).index() <= ($pageDisplay[$currentActivePage - 1].rowEnd -1 )) {
